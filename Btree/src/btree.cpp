...
// if another scan is on going, end that scan
        ...
        //std::cout << "unpin at line " << __LINE__ << ", going to unpin page " << currentPageNum << std::endl;
...
// -----------------------------------------------------------------------------
    ...
    const void BTreeIndex::scanNext(RecordId& outRid)
    {
            ...
            //std::cout << "key is " << key << " at line " << __LINE__ << std::endl;
            if(!checkValid(key))
            {
            ...
            if(leafnode->rightSibPageNo == 0 &&  nextEntry == INTARRAYLEAFSIZE-1)
            {
                throw IndexScanCompletedException();
            }
            //std::cout << "nextEntry is " << nextEntry << std::endl;
            if(nextEntry < INTARRAYLEAFSIZE - 1)
            {
                //std::cout << "nextEntry is less than LEAFSIZE - 1" << std::endl;
                //std::cout << "nextEntry + 1 = " << nextEntry + 1 << std::endl;
                if(leafnode->keyArray[nextEntry + 1] == 0)
                {
                    //std::cout << "key[nextEntry + 1] (should == 0) = " << leafnode -> keyArray[nextEntry + 1] << std::endl;
                    nextEntry = 0;
                    //std::cout << "currentPageNum = " << currentPageNum << std::endl;
                    //test
                    //Page* testPage;
                    //bufMgr -> readPage(file, currentPageNum, testPage);
                    //LeafNodeInt* testLeaf = (LeafNodeInt*)testPage;
                    //std::cout << "testLeaf -> rightSibPageNo = " << testLeaf -> rightSibPageNo << std::endl;
                    //bufMgr -> unPinPage(file, currentPageNum, testPage);
                    //end
                    currentPageNum = leafnode->rightSibPageNo;
                    //std::cout << "rightsibpageno = " << leafnode -> rightSibPageNo << std::endl;
                    //std::cout << "(updated)currentPageNum = " << currentPageNum << std::endl;
                ...
                {
                    //std::cout << "curr page in line " << __LINE__ << " is " << currentPageNum << std::endl;
                    //std::cout << "key[nextEntry + 1] (should != 0) = " << leafnode -> keyArray[nextEntry + 1] << std::endl;
                    LeafNodeInt* p = (LeafNodeInt *) currentPageData;
                    outRid = p ->ridArray[nextEntry];
                    //std::cout << "nextEntry before increment is " << nextEntry << std::endl;
                    nextEntry = nextEntry + 1;
                }
            }
            else
            {
                //std::cout << "nextEntry is larger than LEAFSIZE - 1" << std::endl;
                nextEntry = 0;
                //std::cout << "(larger) currentPageNo = " << currentPageNum << std::endl;
                currentPageNum = leafnode->rightSibPageNo;
                //std::cout << "(larger) rightSibPageNo = " << leafnode -> rightSibPageNo << std::endl;
                //std::cout << "(larger update) currentPageNo = " << currentPageNum << std::endl;
                bufMgr -> readPage(file, currentPageNum, currentPageData);
                LeafNodeInt* p = (LeafNodeInt *) currentPageData;
                outRid = p->ridArray[0];
                std::cout << "unpin at line " << __LINE__ << ", going to unpin page " << currentPageNum << std::endl;
...
//
    const void BTreeIndex::endScan()
    {
        ...
        scanExecuting = false;
        // unpin
        std::cout << "curr pageNo in FUNCTION " << __FUNCTION__ << " is " << currentPageNum << std::endl;
        std::cout << "root pageNo in FUNCTION " << __FUNCTION__ << " is " << rootPageNum << std::endl;
        std::cout << "unpin at line " << __LINE__ << ", going to unpin page " << currentPageNum << std::endl;
        bufMgr -> unPinPage(file, rootPageNum, false);
    ...
    const void BTreeIndex::search_key_in_leaf(LeafNodeInt* LeafNode, int PageNum)
    {
        ...
        for(int i = 0;i < INTARRAYLEAFSIZE; i++) {
            //std::cout<< "i :" << i <<" , key is " <<  LeafNode -> keyArray[i] << std::endl;
            //std::cout << "i: " << i << ", pageNo: " << PageNum << std::endl;
        ...
        std::cout << "unpin at line " << __LINE__ << ", going to unpin page " << PageNum << std::endl;
